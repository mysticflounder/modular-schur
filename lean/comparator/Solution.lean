...
`Headline.` name and proving it from the real project theorem.

Each `Headline.*` statement is the mathlib-only inlining of a project theorem
...
`Headline.*` theorem's `#print axioms` closure is `{propext, Classical.choice,
