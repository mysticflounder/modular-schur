...
\begin{abstract}
The modular Schur number $S_m(k,\ell)$ of D'orville--Sim--Wong--Ho~\cite{DSWH2025} is the greatest $N$ such that $[1,N]$ admits a $k$-partition into classes that are $\ell$-sum-free modulo~$m$. We prove that
\[
S_m(k,\ell) \;=\; \frac{m}{\gcd(m,\ell-1)} - 1
\]
for every $m\ge 2$, $\ell\ge 2$, and $k\ge m/\gcd(m,\ell-1) - 1$. The proof is two paragraphs of modular arithmetic. It recovers the per-modulus tables of~\cite{DSWH2025} at $m\le 7$ and extends them to all $m$. The main theorem is formalized in Lean~4 with axiom dependencies only \texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}. We also give a sharp $k_0$ for prime moduli and record a small composite counterexample ($C=\{1,5\}\subseteq\Zm[12]$) that refutes the natural ``only singletons are safe'' conjecture.
\end{abstract}

\section{Introduction}

The classical Schur number $S(k)$ is the largest $N$ such that $\{1,\ldots,N\}$ admits a $k$-partition into sum-free sets. Values are known only through $S(5) = 160\,537$~\cite{Heule2017}; $S(6)$ is the content of Erd\H{o}s problem~483 and remains open.

D'orville, Sim, Wong, and Ho~\cite{DSWH2025} recently introduced the modular generalization.
...
In~\cite{DSWH2025} complete formulas are given for $m\in\{1,\ldots,7\}$ by residue-by-residue case analysis on $\ell\bmod m$. The pattern visible across those seven tables---once $k$ is large enough, $S_m(k,\ell)$ is a function of $\gcd(m,\ell-1)$ alone---was conjectural for general~$m$.
